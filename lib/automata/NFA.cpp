...
DFA NFA::toDFA() {
    ...
    while (!pending_states.empty()) {
        ...
        set_intersection(state_set.begin(), state_set.end(), accepting_states.begin(), accepting_states.end(),
...
NFA::NFA(int size, int initial_state, map<pair<int, char>, set<int>> transitions, const set<int> &accepting_states) {
    ...
    for (map_it = transitions.begin(); map_it != transitions.end(); ++map_it) {
        ...
        this->transitions.insert(
                pair<pair<int, char>, set<int>>(make_pair(tmp_dic[map_it->first.first], map_it->first.second),
                                                transition_states));
    }
...
NFA NFA::lexical_union(const vector<NFA> &nfas) {
    NFA result;
    result.initial_state = NFA::getStateNumber();
